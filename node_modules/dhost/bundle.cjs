...
var isPosix = path.sep === path.posix.sep;
var noop = (x) => x;
if (!isPosix && path.sep !== "\\") {
  throw new Error(`can't start, unknown path.sep: ${path.sep}`);
}
var posixToPlatform = isPosix ? noop : (x) => x.replace(/\//g, "\\");
...
function reallink(filename) {
  return __async(this, null, function* () {
    let curr = filename;
    for (; ; ) {
      const link = yield readlinkOrNull(curr);
      if (link === null) {
        return curr;
      } else if (path2.isAbsolute(link)) {
        curr = link;
      } else {
        curr = path2.join(path2.dirname(curr), path2.sep, link);
      }
    }
  });
}
function asyncWrite(w, buffer) {
  return __async(this, null, function* () {
    const p = new Promise((resolve2, reject) => {
      w.write(buffer, (err) => err ? reject(err) : resolve2());
    });
    return p;
  });
}
...
function realpathIn(root, pathname) {
  return __async(this, null, function* () {
    const hasTrailingSep = pathname.endsWith(path2.sep);
    const parts = splitPath(posixToPlatform(path2.posix.normalize(pathname)));
    let curr = root;
    for (const part of parts) {
      if (!part.length) {
        continue;
      }
      const test = path2.join(curr, path2.sep, part);
      curr = yield reallink(test);
      if (curr !== test && !pathInRoot(root, curr)) {
        return null;
      }
    }
    return path2.join(curr, hasTrailingSep ? path2.sep : "");
  });
}
function statOrNull(filename, lstat2 = false) {
  return __async(this, null, function* () {
    const method = lstat2 ? fs.lstat : fs.stat;
...
function pathInRoot(root, cand) {
  while (root.endsWith(path2.sep)) {
    root = root.slice(0, -path2.sep.length);
  }
  if (!cand.startsWith(root)) {
    return false;
  }
  const check = cand.substr(root.length, path2.sep.length);
  return check.length === 0 || check === path2.sep;
}
...
function getType(filename) {
  ...
  return import_mime.default.getType(filename);
}
...
function writeContentType(res, filename, contentType = void 0) {
  contentType = contentType || getType(filename) || void 0;
  if (contentType) {
...
// lib/handler.js
var READ_SYNC_THRESHOLD = 64 * 1024;
function buildHandler(rawOptions = ".") {
  ...
  const rootPath = path3.resolve(options.path);
  ...
  const handler = (req, res, next) => __async(this, null, function* () {
    ...
    const normalized = path3.posix.normalize(pathname);
    if (pathname !== normalized) {
      return redirect(res, normalized);
    }
    const rel = relativePath(req);
    if (!options.serveHidden && rel.includes("/.")) {
      res.writeHead(404);
      return res.end();
    }
    let filename = path3.join(rootPath, ".", posixToPlatform(rel));
    if (redirectToLink) {
      const real = yield realpathIn(rootPath, rel);
      if (real === null) {
        res.writeHead(403);
        return res.end();
      }
      if (real !== filename) {
        ...
        if (!hasTrailingSlash) {
          filename = path3.dirname(filename);
        }
        const suffix = hasTrailingSlash ? "/" : "";
        return redirect(res, path3.relative(filename, real) + suffix);
      }
    }
    let stat2 = yield statOrNull(filename);
    if (stat2 && req.method === "HEAD") {
      return res.end();
    }
    if (stat2 == null ? void 0 : stat2.isDirectory()) {
      const cand = path3.join(filename, "index.html");
      const indexStat = yield statOrNull(cand, redirectToLink);
      if (indexStat && !indexStat.isDirectory() && !indexStat.isSymbolicLink()) {
        filename = cand;
        stat2 = indexStat;
      } else if (!rel.endsWith("/")) {
        const dir = path3.posix.basename(rel);
        return redirect(res, `${dir}/`);
      }
    }
    const arg = { stat: stat2, filename, pathname, search };
    for (const rw of options.rewriters) {
      const ret = yield rw(arg);
      if (!ret) {
        continue;
      }
      if (!stat2 && req.method === "HEAD") {
      ...
      writeContentType(res, filename, ret.contentType);
      res.setHeader("Content-Length", ret.buffer.length);
      yield asyncWrite(res, ret.buffer);
      return res.end();
    }
    ...
    if (rangeHeader) {
      readOptions = parseRange(rangeHeader, stat2.size);
      if (!readOptions) {
        res.setHeader("Content-Range", `bytes */${stat2.size}`);
        res.writeHead(416);
        return res.end();
      }
      res.setHeader("Content-Range", `bytes ${readOptions.start}-${readOptions.end - 1}/${stat2.size}`);
      res.setHeader("Content-Length", readOptions.end - readOptions.start);
    } else {
      res.setHeader("Content-Length", stat2.size);
    }
    writeContentType(res, filename);
    res.setHeader("Accept-Ranges", "bytes");
    if (stat2.size <= READ_SYNC_THRESHOLD) {
      let buffer = fs2.readFileSync(filename);
      if (readOptions) {
        buffer = buffer.slice(readOptions.start, readOptions.end);
        res.writeHead(206);
      }
      yield asyncWrite(res, buffer);
      return res.end();
    }
    const readStream = fs2.createReadStream(filename, readOptions);
